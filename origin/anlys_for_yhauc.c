...
{
    FILE* fd = NULL;
    char buff[BUFFERSIZE*5];
    char tmpbuff[BUFFERSIZE*5];
    ...
    */
    ...
    fd = fopen("tmp","rw+");
    fseek(fd,BUFFERSIZE*39-5000,SEEK_CUR); /*get over the garbages at the beginning of a html file*/
    ...
    /*Read a crawled html*/
    if(SIZE*BUFFERSIZE*5 != fread(buff,SIZE,BUFFERSIZE*5,fd))
    {
        printf("\n fread() failed\n");
        return 1;
    }
	...
	cursor = strstr(buff, "a1wrp");
    	...
    	copywith(tmpbuff,cursor+ALIGNMENT+len,BUFFERSIZE*5-offset);/*Get rid of the already processed texts*/
    	//fprintf(stdout,"%s\n",tmpbuff);
    	...
    	copywith(buff,cursor+ALIGNMENT+len,BUFFERSIZE*5-offset);/*Get rid of the already processed texts*/
    }
  
  
    /*  Test for dividing
    	divider = strstr(linklist[10],"/jp");
    	int offset = (int)(divider-linklist[10]);
    	copywith(host,linklist[10],offset);
   ...
   */
	/*Find the pages fulfilling our requests*/
	//judge_page(linklist[2],"入札者評価制限");
	int res;
